...
function print_usage_and_exit {
	...
	echo "|-> -i: start the script in interactive mode (default non-interactive)"
	echo "|-> -e: specify that the connection must be encrypted (default 0)"
	echo "|-> -d: specify the resolution to be used (default actual screen dimensions)"
	echo "|-> -t: connection/wait timeout in seconds (default 60s)"
	echo "|-> -p: connection protocol to be used (default vnc, supports also xrdp and novnc)"
	echo "|"
