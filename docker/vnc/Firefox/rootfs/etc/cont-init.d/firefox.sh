...
if [ "$SHM_SIZE_MB" -eq 64 ]; then
   echo 'FAIL' > /config/log/firefox/firefox_shm_check
else
   echo 'PASS' > /config/log/firefox/firefox_shm_check
fi

# Fix window display size is session stores.
if [ -n "$(ls /config/.mozilla/profile/sessionstore-backups/*.jsonlz4 2>/dev/null)" ]; then
    for FILE in /config/.mozilla/profile/sessionstore-backups/*.jsonlz4; do
        WORKDIR="$(mktemp -d)"
